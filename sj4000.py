...
# pylint: disable=format-string-without-interpolation, consider-using-f-string
...
class Camera:
    ...
    def get_config_by_name(self, config):
        """
        Function to take config name and return config id
        """
    ...
    def get_element(self, response, element):
        """
        Function to extract a single element from response to 'send_command'
        """
    ...
    def get_file(self, path, fname):
        """
        Functiono to take file path and return file
        """

        ret, flen = self.print_directory(quiet = True, find = fname)
        if not ret:
            return False, "Could not read directory"
        if not flen:
            return False, "Could not get file length"
        resp = requests.get("http://" + self.ip + fname, stream =True)
        ...
        for chunk in resp.iter_content(chunk_size = 2048 * 1024):
            if chunk: # filter out keep-alive new chunks
                gotlen += len(chunk)
                print(self.human_readable(gotlen), " of ", flen, "\r")
                sys.stdout.flush()
                outfile.write(chunk)
        outfile.close()
        resp.close()
    ...
    def get_file_details(self, cells, filename):
        """
        Function to return file length and creation date/time from html table
        """

        for cell in enumerate(cells):
            try:
                entry = cells[cell].findChildren('a')[0].get('href')
            except Exception as exception:
    ...
    def get_mode(self):
        """
        Function to return status mode
        """
    ...
    def get_preview(self):
        """
        Function to get low res image

        Coment from dev:
        not sure what's going on here, but we seem to be able to grab a low res
        image when in PHOTO mode
        """
        ...
        for _ in range(3):
            header = resp.raw.readline()
            if header.startswith('Content-length:'):
                size = int(header.split(' ')[1])
        if not size:
            return False, 'Could not determine image size!'
        data = resp.raw.read(size)
        resp.close()
        return True, data

    def http_test(self):
        """
        Function to test HTTP socket
        """
    ...
    def human_readable(self, num):
        """
        Function to convert number to human readable format
        """

        units = ['B','KB','MB','GB','TB','PB','EB','ZB','YB']
        power = math.floor(math.log(num, 2)/10)
        hrnum = num/(1024**power)
        return "%.2f %s".format(hrnum, units[int(power)])
    ...
    def print_config_help(self, parameter):
        """
        Function to print config help
        """

        if parameter:
            print('    ', self.CONFIG[self.get_config_by_name(parameter)][0],':')
            print(', '.join(list(self.CONFIG[self.get_config_by_name(parameter)][1:])))
        else:
            for item in self.CONFIG.items()[1]:
                print('    ', format(item[0]), ':')
        ...
        if ret:
            ...
            for branch in tree:
                if branch.tag == 'Status':
                    ...
                    current = branch.text
                    try:
                        print('    ', self.CONFIG[branch.text][0], ':')
                    except Exception as exception:
                        print("[WARN]: Exception occured, ", exception)
                        print('    ', branch.text, ': ')
        else:
    ...
    def print_directory(self, quiet = False, find = None):
        """
        Function to print directory and get disk space
        """
        ...
        for thing in "PHOTO", "MOVIE":
            if not quiet:
                ...
                print()
            try:
                resp = requests.get("http://" + self.ip + "/DCIM/" + thing, timeout = 5)
            except Exception as exception:
                print("[ERROR]: Exception occured, ", exception)
    ...
    def send_command(self, command, param = None, str_param = None):
        """
        Function to send command by name or number
        """
    ...
    def set_wifi_name(self, name):
        """
        Funtion to set wifi name
        """

        return self.send_command('WIFI_NAME', str_param = name)

    def set_wifi_pw(self, pwparam):
        """
        Function to set wifi password
        """
    ...
    def snap(self, path):
        ...
        found = soup.find_all('a')[len(soup.find_all('a')) - 2].get('href')
        fname = found.split('/')[-1:][0]
        if path:
            return self.get_file(path, found)
        return True, fname

    def start_stop_movie(self, action):
        """
        Function to start and stop movie recording
        """
    ...
    def stream(self):
        ...
        subprocess.Popen(['vlc', 'rtsp://' + self.ip + '/sjcam.mov'], stdout = self.dev_null, stderr = subprocess.STDOUT)
